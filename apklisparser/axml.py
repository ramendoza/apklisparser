...
log = logging.getLogger("ApklisParser:AXML")
...
class StringBlock(object):
    ...
    def decodeLength(self, offset, sizeof_char):
        ...
        if (length & highbit) != 0:
            return ((length1 & ~highbit) <<
                    (8 * sizeof_char)) | length2, sizeof_2chars
        else:
    ...
    def show(self):
        print(
            "StringBlock(%x, %x, %x, %x, %x, %x" % (
                self.start, self.header, self.header_size, self.chunkSize,
                self.stringsOffset, self.flags
            )
        )
...
class AXMLParser(object):
    def __init__(self, raw_buff):
        ...
        header = ARSCHeader(
            self.buff
        )  # read 8 byte = String header + chunk_size
...
class ARSCParser(object):
    ...
    def __init__(self, raw_buff):
        ...
        while self.buff.get_idx() <= data_end - ARSCHeader.SIZE:
            ...
            if res_header.type == RES_STRING_POOL_TYPE and not self.stringpool_main:
                ...
                pc = PackageContext(
                    current_package, self.stringpool_main, mTableStrings,
                    mKeyStrings
                )
                ...
                self.buff.set_idx(
                    res_header.start + res_header.header_size +
    ...
    def get_resource_dimen(self, ate):
        try:
            ...
            log.debug(
                "Out of range dimension unit index for %s: %s" % (
                    complexToFloat(ate.key.get_data()),
        ...
        def put_ate_value(self, result, ate, config):
            if ate.is_complex():
                ...
                for _, item in ate.item.items:
                    self.put_item_value(
                        complex_array, item, config, complex_=True
                    )
            else:
...
class ARSCResStringPoolRef(object):
    ...
    def __repr__(self):
        return "<ARSCResStringPoolRef idx='0x{:08x}' size='{}' type='{}' data='0x{:08x}'>".format(
            ...
            TYPE_TABLE.get(self.data_type, "0x%x" % self.data_type), self.data
        )
